...
class YouTubeService {
  ...
  async searchTracks(query, maxResults = 25, pageToken = "") {
    const cacheKey = cache.generateKey("youtube:search:tracks", {
      query,
      maxResults,
      pageToken,
    });
  ...
  async getTrackDetails(videoId) {
    ...
    try {
      ...
      logger.error(
        "YouTube track details error:",
        error.response?.data || error.message
      );
      throw new ErrorHandler(
        "Failed to fetch track details",
        500,
        "YOUTUBE_DETAILS_ERROR"
      );
    }
  }

  async getTrendingTracks(maxResults = 25, regionCode = "IN") {
    const cacheKey = cache.generateKey("youtube:trending", {
      maxResults,
      regionCode,
    });
    const cached = await cache.get(cacheKey);
  ...
  async getStreamingUrl(videoId) {
    ...
    try {
      ...
      try {
        ...
        logger.warn(
          "Could not get video info, using default duration:",
          infoError.message
        );
  ...
  getMockSearchResults(query, maxResults = 25) {
    ...
    const limitedTracks = mockTracks.slice(
      0,
      Math.min(maxResults, mockTracks.length)
    );
  ...
  getMockTrackDetails(videoId) {
    return {
      ...
      artist: "Demo Artist",
      description:
        "This is a demo track returned when YouTube API key is not configured. Add your YouTube API key to the .env file to get real data.",
      thumbnail:
        "https://via.placeholder.com/1280x720?text=Demo+Track+Thumbnail",
      duration: 215,
      publishedAt: "2025-01-15T10:00:00Z",
      viewCount: 1000000,
      likeCount: 25000,
